---
enable: true
testimonials:
  - name: Antoine Sebag
    avatar: "/images/avatars/avatar-1.jpg"
    company:
      name: "Directeur commercial"
      link: "#"
    content: Leur solution de gestion de la lead Gen nous a permis de signer 28% de contrats en plus sur l'année. Leur accompagnement et leur compréhension du business a été un vrai plus !!! Je recommande.

  - name: Isabelle Retondeau
    avatar: "/images/avatars/avatar-2.jpg"
    company:
      name: "Ostéopathe"
      link: "#"
    content: Merci à Emmanuel d'Ohtomation d'avoir mis en place notre nouvelle interface de gestion des RDV. Grâce aux rappels automatisés par mail, texto et téléphone, mon cabinet a pu diviser par 3 l'absentéisme.

  - name: Gauthier Fournier
    avatar: "/images/avatars/avatar-3.png"
    company:
      name: "Avocat"
      link: "#"
    content: Grâce à Ohtomation j'ai pu gagner 3 jours par mois dans la classification et l'ordonnancement de mes dossiers au cabinet.

  - name: Frédéric Schneider
    avatar: "/images/avatars/avatar-1.jpg"
    company:
      name: "Directeur marketing"
      link: "#"
    content: La solution de création de contenu proposée par Emmanuel nous a permis de passer de 3K abonnés à 12,5K abonnés en 4 mois sur LinkedIn.

  - name: Jérôme Boulonnois
    avatar: "/images/avatars/avatar-2.jpg"
    company:
      name: "Bureau d'études et d'ingénierie"
      link: "#"
    content: Nous avons fait appel à Ohtomation pour fluidifier notre processus d'inventaire dans le cadre de notre croissance. La solution qui a été mise en place pour notre équipe sur le terrain est connectée à notre catalogue en ligne sur Shopify. Résultat — notre équipe a récupéré près de 40 % de son temps.
---
